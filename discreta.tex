...
\begin{ese}
    \begin{figure}[!h]
        ...
        \end{tikzpicture}
    \end{figure}
Il grafo non orientato nella figura precedente non è semplice poiché presenta un
...
\begin{defn}[cammino]
Un \emph{cammino} è una sequenza di vertici \emph{distinti} dove ogni coppia di vertici consecutivi
è uno spigolo.
\end{defn}
...
\end{defn}
\begin{ese}
$v_4 \text{ - } v_7 \text{ - } v_5 \text{ - } v_3 \text{ - } v_2 \text{ - } v_4$ è un circuito.
    \begin{figure}[!h]
    \centering
        \begin{tikzpicture}
            \node[shape=circle,draw=black]  (A) at (0,6) {$v_1$};
...
\[A = \{ (v_1, v_2), (v_1, v_3), (v_2, v_5), (v_3, v_5), (v_4, v_1), (v_4, v_3), (v_5, v_2) \} \]
    \begin{figure}[!h]
    \centering
        \begin{tikzpicture}
            \node[shape=circle,draw=black] (A) at (0,0) {$v_1$};
...
il grafo ha un \emph{nodo iniziale} ed un \emph{nodo finale}.
...
\begin{ese}
L'immagine sottostante non rappresenta un grafo perchè i vertici $v_1$ e $v_2$
sono collegati da due archi paralleli (ovvero due archi che hanno lo stesso nodo
iniziale ed anche quello finale), inoltre c'è un cappio su $v_1$.
    \begin{figure}[!h]
    \centering
        \begin{tikzpicture}
            \node[shape=circle,draw=black] (A) at (0, 0) {$v_1$};
...
\begin{ese}
Nel grafo in figura è presente il cammino orientato: \[v_1 - v_3 - v_5 - v_2\]
    \begin{figure}[!h]
    \centering
        \begin{tikzpicture}
...
\begin{ese}
Circuito orientato: \[v_1 - v_3 - v_5 - v_2 - v_1\]
    \begin{figure}[!h]
    \centering
        \begin{tikzpicture}
            \node[shape=circle,draw=orange] (A) at (0,0) {$v_1$};
